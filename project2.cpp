...
* ********************************************************** */
#define NUM_REPEAT 10 // each process repeats 
#define READER_TIME_01 300 // 300ms = 0.3 seconds
#define READER_TIME_02 800 // 800ms = 0.8 seconds
#define WRITER_TIME_01 1200 // 1200ms = 1.2 seconds
#define WRITE_TIME_02 1600 // 1600ms = 1.6 seconds
...
struct my_mem {
    int reader_count = 0;
    int writer_finish_status[2] = {1, 1};
    int writer_waiting_status[2] = {0, 0};
    char posted_msg[256] = "No one has posted a message.";
};
...
union semun mutex2;
union semun mutex3;
union semun counting1;
union semun counting2;
...
void reader(int id){
    ...
    while(shmp->writer_finish_status[0] == 1 || shmp->writer_finish_status[1] == 1){
        ...
        printf("Reader %d likes to read the posted message.", id);
        ...
        printf("Reader %d starts reading the message:", id);
        printf("%s\n", shmp->posted_msg);
        
        sleep_time = READER_TIME_02;
        millisleep(sleep_time);
        printf("Reader %d finishes reading the posted message.", id);
...
void writer(int id){
    ...
    int sleep_time;
    char my_message[256];
    strcpy(my_message, "Hello, I am writer %d... (loop 0)", id);
    ...
    for(int i = 0; i < NUM_REPEAT; i++){
        ...
        shmp-> writer_status[id-1] = 1; //Set writer status to 1 (waiting to write)
        printf("Writer %d likes to post its message.", id);
        ...
        //Start of the critical section
        printf("Writer %d starts posting a new message:", id);
        printf("%s\n", my_message);
        strcpy(shmp->posted_msg, my_message)
        sleep_time = WRITE_TIME_02;
        millisleep(sleep_time);

        printf("Writer %d finishes posting a new message.", id);
        //End of the critical section

        my_message[30] = i + 1; //Increment the loop number in the message (at position 30)
